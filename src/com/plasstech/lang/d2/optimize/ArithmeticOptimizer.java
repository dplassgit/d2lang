...
class ArithmeticOptimizer extends LineOptimizer {
  ...
  private static final List<OpcodeOptimizer> BINOP_OPTIMIZERS =
      ImmutableList.of(
          ...
          new BinOpLeftRightOptimizer(
              ImmutableList.of(TokenType.EQEQ, TokenType.LEQ, TokenType.GEQ),
              transferFrom(ConstantOperand.TRUE)),
          // x != x => false
          new BinOpLeftRightOptimizer(
              ImmutableList.of(TokenType.NEQ, TokenType.LT, TokenType.GT),
          ...
          new BinOpOptimizer(
              ...
              op -> {
                BinOp binop = (BinOp) op;
                return new UnaryOp(
                    op.getDestination(), TokenType.MINUS, binop.left(), op.position());
              }),
          // a * (power of 2) -> a << (log of power of 2)
          new BinOpOptimizer(
              Matchers.hasType(VarType.INT),
              TokenType.MULT,
              Matchers.isPowerOf2(),
              op -> {
                BinOp binop = (BinOp) op;
          ...
          new BinOpOptimizer(
              Matchers.any(),
              ImmutableList.of(TokenType.MULT, TokenType.BIT_AND),
              Matchers.isAnyZero(),
              transferFromRight()),
          ...
          new BinOpOptimizer(
              Matchers.isAnyZero(),
              TokenType.DIV,
              Matchers.not(Matchers.isAnyZero()),
              transferFromLeft()),
          // a=b*1 or b/1 -> a=b
          new BinOpOptimizer(
              Matchers.any(),
          ...
          // x - x = 0
          new BinOpLeftRightOptimizer(
              ImmutableList.of(TokenType.MINUS, TokenType.MOD), transferFromZero()),
          // x / x = 1
          new BinOpLeftRightOptimizer(ImmutableList.of(TokenType.DIV), transferFrom(1)),
          // x + x = x<<1, only for int
          new BinOpLeftRightOptimizer(
              ImmutableList.of(TokenType.PLUS),
              op -> {
                BinOp binop = (BinOp) op;
          ...
          new BinOpOptimizer(
              ...
              op -> {
                BinOp binop = (BinOp) op;
                return new UnaryOp(
                    op.getDestination(), TokenType.MINUS, binop.right(), op.position());
              }),
          // a = b + (-C) => a = b - C
          new BinOpOptimizer(
              Matchers.isNumeric(),
              ...
              op -> {
                ...
                long value = ConstantOperand.valueFromConstOperand(binop.right()).longValue();
                return new BinOp(
                    op.getDestination(),
                    binop.left(),
                    TokenType.MINUS,
                    ConstantOperand.fromValue(-value, binop.right().type()),
                    op.position());
              }),
          // a = b - (-C) => a = b + C
          new BinOpOptimizer(
          ...
          new BinOpOptimizer(
              ...
              op -> {
                ...
                int index = ConstantOperand.valueFromConstOperand(binop.right()).intValue();
                return new Transfer(
                    op.getDestination(), ConstantOperand.of(range.value(index)), op.position());
              }),
  ...
  private static final List<OpcodeOptimizer> UNOP_OPTIMIZERS =
      ImmutableList.of(
          // length(range) is always 2
          new UnaryOpOptimizer(
              Matchers.hasType(VarType.RANGE),
              TokenType.LENGTH,
          ...
          new UnaryOpOptimizer(
              ...
              op -> {
                ...
                return new Transfer(
                    op.getDestination(), ConstantOperand.of(s.length()), op.position());
              }),
          ...
          new UnaryOpOptimizer(
              ...
              op -> {
                ...
                return new Transfer(
                    op.getDestination(),
                    ConstantOperand.fromValue(-operand, unop.operand().type()),
                    op.position());
              }),
          new UnaryOpOptimizer(
              Matchers.isDoubleConstant(),
              ...
              op -> {
                UnaryOp unop = (UnaryOp) op;
                double operand =
                    ConstantOperand.valueFromConstOperand(unop.operand()).doubleValue();
                return new Transfer(
                    op.getDestination(), ConstantOperand.of(-operand), op.position());
              }),
          ...
          new UnaryOpOptimizer(
              ...
              op -> {
                ...
                if (value.length() < 1) {
                  throw new InvalidIndexException(op.position(), "Cannot take ASC of empty STRING");
                }
                char first = value.charAt(0);
                return new Transfer(unop.destination(), ConstantOperand.of(first), op.position());
              }));
  ...
  private static Function<Op, Op> optimizeIntBinOp(BinaryOperator<Long> fun) {
    return op -> {
      ...
      return new Transfer(
          op.getDestination(),
          ConstantOperand.fromValue(fun.apply(left, right), binop.left().type()),
          op.position());
    };
  }
  ...
  private static Function<Op, Op> optimizeComparison(
    ...
    return op -> {
      ...
      return new Transfer(
          op.getDestination(), ConstantOperand.of(fun.test(leftval, rightval)), op.position());
    };
  }
  ...
  private static Function<Op, Op> optimizeDoubleBinOp(BinaryOperator<Double> fun) {
    return op -> {
      ...
      return new Transfer(
          op.getDestination(), ConstantOperand.of(fun.apply(left, right)), op.position());
    };
  }
  ...
  private static Function<Op, Op> transferFrom(long value) {
    return op -> {
      ...
      return new Transfer(
          op.getDestination(),
          ConstantOperand.fromValue(value, binop.left().type()),
          op.position());
    };
  }
