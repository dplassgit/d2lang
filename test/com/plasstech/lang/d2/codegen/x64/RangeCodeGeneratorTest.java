...
public class RangeCodeGeneratorTest {
  ...
  private static final Location PARAM1 =
      LocationUtils.newParamLocation("param", VarType.INT, 0, 0);
  private static final Location PARAM2 =
      LocationUtils.newParamLocation("param2", VarType.INT, 5, 16);
  private static final Location LOCAL1 =
      LocationUtils.newStackLocation("local1", VarType.INT, 16);
  private static final Location LOCAL2 =
      LocationUtils.newStackLocation("local2", VarType.INT, 32);
  ...
  public void createRangeParams() {
    ...
    assertThat(code).containsExactly("mov DWORD EBX, ECX", "shl QWORD RBX, 32",
        "mov DWORD ESI, [RBP + 16]", "add RBX, RSI");
  }

  @Test
  public void createRangeToParam() {
    BinOp op =
        new BinOp(PARAM_RANGE, LOCAL1, TokenType.COLON, LOCAL2, null);
    ImmutableList<String> code = generateUncommentedCode(op);
    assertThat(code).hasSize(4);
    assertThat(code).containsExactly("mov DWORD EDX, [RBP - 16]", "shl QWORD RDX, 32",
        "mov DWORD EBX, [RBP - 32]", "add RDX, RBX");
  }

  @Test
  public void createRangeToLocal() {
    BinOp op =
        new BinOp(LOCAL_RANGE, LOCAL1, TokenType.COLON, LOCAL2, null);
    ImmutableList<String> code = generateUncommentedCode(op);
  ...
  public void createRangeFromParams(@TestParameter boolean optimize) {
    assertThatCompiling(
        "f:proc(start:int, end:int, index:int){r=start:end print r[index]} f(3, 5, 0) f(4, 6, 1)")
  ...
  public void createRangeFromLocals(@TestParameter boolean optimize) {
    assertThatCompiling(
        "f:proc(index:int){start=3 end=5 r=start:end print r[index]} f(0) f(1)")
        .withOptimize(optimize)
        .executedEqualsInterpreted();
  }
  ...
  public void createRangeFromGlobals(@TestParameter boolean optimize) {
    assertThatCompiling(
        "start=3 end=5 f:proc(index:int){r=start:end print r[index]} f(0) f(1)")
        .withOptimize(optimize)
        .executedEqualsInterpreted();
  }
