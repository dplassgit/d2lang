...
public class NasmCodeGeneratorIntTest {
  ...
  public void intUnaryOps(@TestParameter({"-", "!"}) String op) throws Exception {
    assertThatCompiling(String.format("a=3 b=%sa println b", op))
    ...
    assertThatCompiling(
            String.format(
                "      a=%d b=%d "
                    ...
                    + "dp(a,b) "
                    + "ep()",
                first, second, op, op, first, second, op))
        .withOptimize(optimize)
        .executedEqualsInterpreted();
  }
  ...
  public void shiftOpsProc(@TestParameter({"<<", ">>"}) String op, @TestParameter boolean optimize)
    ...
    String program =
        String.format(
            "f:proc(a:int) {b=4 b=b %s a println a a=a %s b println a c=a<<2 println c} f(2)",
            op, op);
    assertThatCompiling(program).withOptimize(optimize).executedEqualsInterpreted();
  }

  @Test
  public void shiftOpsGlobal(
      @TestParameter({"<<", ">>"}) String op, @TestParameter boolean optimize) throws Exception {
    String program =
        String.format("a=2 b=4 b=b %s a println a a=a %s b println a c=a<<2 println c", op, op);
    assertThatCompiling(program).withOptimize(optimize).executedEqualsInterpreted();
  }
  ...
  public void shiftParamInExpression(@TestParameter boolean optimize) throws Exception {
    assertThatCompiling("f:proc(b: int, a:int) {a=(b>>2) + a << (a+b) println a} f(256, 2)")
        .withOptimize(optimize)
        .executedEqualsInterpreted();
  }
  ...
  public void shiftSelfParam(@TestParameter boolean optimize) throws Exception {
    assertThatCompiling("f:proc(a:int) {a=a << a println a} f(2)")
        .withOptimize(optimize)
        .executedEqualsInterpreted();
  }
  ...
  public void shiftSelfLocal(@TestParameter boolean optimize) throws Exception {
    assertThatCompiling("f:proc(a:int) {b=a+1 b=b << b println b} f(2)")
        .withOptimize(optimize)
        .executedEqualsInterpreted();
  }
  ...
  public void divLoop() throws Exception {
    assertThatCompiling("a=10000 while a > 0 {println a a = a / 10 }")
        .withOptimize(optimize)
        .executedEqualsInterpreted();
  }
  ...
  @Test
  public void opEquals(@TestParameter({"+=", "-=", "*=", "/="}) String op) throws Exception {
    assertThatCompiling(
            String.format(
                "f:proc(a: int, b:int) { a %s b c=b c %s a println a println c} f(100, 10)",
                op, op))
        .withOptimize(optimize)
        .executedEqualsInterpreted();
  }
