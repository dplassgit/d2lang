...
public class AddSubOptimizerTest {
  ...
  public void subAddRegistersSame(
    ...
    ImmutableList<String> code = ImmutableList.of(
        String.format("  sub %s, 0x80", register),
        String.format("  add %s, 0x80", register));

    List<String> output = optimizer.doOptimize(code);
    assertThat(optimizer.isChanged()).isTrue();
    assertThat(output).containsExactly(
        String.format("; sub %s, 0x80", register),
        String.format("; add %s, 0x80", register)).inOrder();
  }
