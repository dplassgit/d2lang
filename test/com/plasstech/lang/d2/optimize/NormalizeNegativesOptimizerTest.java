...
public class NormalizeNegativesOptimizerTest {
  private Optimizer optimizer = new ArithmeticOptimizer(2);
  ...
  public void ignoresStrings() {
    ImmutableList<Op> input =
        ImmutableList
            .of(new BinOp(STRVAR1, STRVAR2, TokenType.PLUS, ConstantOperand.of("hi"), null));
    optimizer.optimize(input, null);
    assertThat(optimizer.isChanged()).isFalse();
  }
  ...
  public void detectsMinus() {
    ImmutableList<Op> input =
        ImmutableList
            .of(new BinOp(INTVAR1, INTVAR2, TokenType.MINUS, ConstantOperand.of(-2), null));
    ImmutableList<Op> optimized = optimizer.optimize(input, null);
    assertThat(optimizer.isChanged()).isTrue();
  ...
  public void detectsPlus_long() {
    ImmutableList<Op> input =
        ImmutableList
            .of(new BinOp(LONGVAR1, LONGVAR2, TokenType.MINUS, ConstantOperand.of(-2L), null));
    ImmutableList<Op> optimized = optimizer.optimize(input, null);
    assertThat(optimizer.isChanged()).isTrue();

    assertThat(optimized).hasSize(1);
    assertThat(optimized.get(0)).isBinOp(LONGVAR1, LONGVAR2, TokenType.PLUS,
        ConstantOperand.of(2L));
  }
  ...
  public void detectsPlus_double() {
    ImmutableList<Op> input =
        ImmutableList
            .of(new BinOp(DBLVAR1, DBLVAR2, TokenType.MINUS, ConstantOperand.of(-2.0), null));
    ImmutableList<Op> optimized = optimizer.optimize(input, null);
    assertThat(optimizer.isChanged()).isTrue();
