...
PlanetType: record {
	...
	x:int y:int		// location (0-100)
	population:double 	// in millions
	status: int 		// 1=occupied, 2=empire, 0=independent
	status_changed:bool	// 1 if status just changed, 0 if not. â€” WHY?!
...
//SuppliesType: record {
 // assets:int[2]	// not sure what this is for
//}
...
names=["Galactica", "Moonsweep", "Ootsi", "Farside", "Sparta", "Proyc", "Novena", "Drassa2", 
	...
	"Harkon", "Bok", "Alhambra"]
NUM_PLANETS=20
// planets: PlanetType[NUM_PLANETS]	// this doesnâ€™t work yet :(
gameinfo:GameInfoType

galmap:int[100*100]		// 0=no planet, else the ascii of the planet

initPlanets:proc {
	// TODO: initialize each planet info with random numbers
    ...
    name = names[i]
		...
		if (i != 0) {
			...
			p.population = 16.0 + tod(random(84)) + 3.0 * tod(random(gameinfo.level))
		} else {
			// Galactica
			p.civ_level = ADVANCED
			p.status = EMPIRE
			p.population = 35.0+10.0*(10.0-tod(gameinfo.level))+tod(random(5))
		...
		galmap[p.x+100*p.y] = asc(p.abbrev)
		//planets[i] = p
	}
}
...
map:proc(planet:PlanetType) {
	...
	y = 0 while y < 24 do y = y + 1 {
		x = 0 while x < 24 do x = x + 1 {
			cell = galmap[x + left_top_x+ 100*(y+left_top_y)]
...
mainLoop: proc {
  ...
  while status==IN_PROGRESS {
    ...
    execute(command)
    sleep(10)
    status = calculate_game_status() // Calc if won or lost
  }
  if status==WON { println "You won" }
  if status==LOST { println "You lost" }
}
